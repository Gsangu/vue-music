...
export default {
  ...
  methods: {
    ...
    searchMore() {
      ...
      search(this.query, this.page, this.showSinger, perpage).then(res => {
        if (res.code === ERR_OK) {
          ...
          // let hash = {};
          // this.result = this.result.reduce((item, next) => {
    ...
    _getResult(data) {
      let ret = [];
      let artist_e = this.result.find(n => {
        return n.singerid === data.zhida.singerid;
      });
      if (data.zhida && data.zhida.singerid && !artist_e) {
