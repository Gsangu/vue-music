        ...
        <div class="bottom">
          ...
          <div class="progress-wrapper">
            ...
            <div class="progress-bar-wrapper">
              <progress-bar :percent="percent" :loading="!songReady" @percentChange="onProgressBarChange"></progress-bar>
            </div>
          ...
          <div class="operators">
            ...
            <div class="icon i-left" :class="disableCss">
              <i @click="prev" class="icon-prev"></i>
            </div>
            <div class="icon i-center" :class="disableCss">
              <i @click="togglePlaying" :class="playIcon"></i>
            </div>
            <div class="icon i-right" :class="disableCss">
...
export default {
  ...
  mounted() {
    ...
    audio
      ...
      .catch(() => {
        this.playError = true;
        let event = ["click", "WeixinJSBridgeReady"];
        let pageClick = e => {
          if (this.playError) {
            this.playError = false;
            audio.load();
  ...
  methods: {
    ...
    getUrl() {
      return this.currentSong.getUrl();
    },
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      const audio = this.$refs.audio;
      this.playingLyric = "加载中...";
      this.songReady = false;
      this.getUrl().then(url => {
        clearTimeout(this.timer);
        this.playingLyric = "加载成功";
        this.timer = setTimeout(() => {
          audio.play().catch(this.error);
          this.getLyric();
        }, 1000); // 从后台切入前台
      }).catch(this.error);
    },
    playing(newPlaying) {
      const audio = this.$refs.audio;
      this.$nextTick(() => {
        newPlaying ? audio.play().catch(this.error) : audio.pause();
      });
    }
  },
  components: {
